...
namespace Itmo.ObjectOrientedProgramming.Lab2.LabworkModel.Entities;

public class Labwork : StudyMaterial
{
    public string Criteria { get; set; } = string.Empty;

    public Labwork(User author, int points, string name)
    {
        if (points < 0)
        {
    ...
    private Labwork(int id) : base(id)
    {
        if (id < 0)
        {
    ...
    {
        var labwork = new Labwork(Id) { IsCopy = true };
        return labwork;
    }
}